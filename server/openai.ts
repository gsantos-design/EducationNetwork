...
/**
 * Get AI tutor response using GPT-5
 * FERPA COMPLIANCE: All student messages are redacted for PII before sending to OpenAI
 */
export async function getTutorResponse(
  ...
  try {
    const systemMessage: TutorMessage = {
      role: "system",
      content: TUTOR_SYSTEM_PROMPT + (subject ? `\n\nCurrent Subject: ${subject}` : "") + (topic ? `\nCurrent Topic: ${topic}` : "")
    };

    // FERPA COMPLIANCE: Redact PII from all user messages before sending to OpenAI
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [systemMessage, ...redactedMessages],
      max_completion_tokens: 8192,
    });

    const responseContent = completion.choices[0]?.message?.content || "I apologize, but I'm having trouble formulating a response. Please try rephrasing your question.";
...
Be specific and constructive in your assessment.`;
    ...
    const completion = await openai.chat.completions.create({
      model: "gpt-5",
      messages: [
        { role: "system", content: "You are an educational assessment expert analyzing tutoring sessions for advanced 9th grade students." },
